...
**Dependencies:** Story 3.1 (Content Type Detection), Story 3.2 (Content-Aware Weighting), Story 8.1 (Configuration Over Code) - thresholds defined in YAML
...
### AC1: Content-Type Thresholds from ConfigRegistry (via Story 8.1)
- [ ] Threshold profiles defined in `config/base.yaml` under `content_types.thresholds` (NOT hardcoded Python dict)
- [ ] Access thresholds via `ConfigRegistry.get(ContentTypeThresholdsConfig)`
- [ ] Includes scoring curves for all 9 content types
- [ ] Each dimension specifies EXCELLENT, GOOD, POOR ranges by content type
- [ ] Ranges are arrays: `[min, max]` for continuous metrics
- [ ] Thresholds reflect genre conventions from CONTENT-TYPE-DIMENSION-MAPPING.md
- [ ] Validation handled by Pydantic schema (min < max, no overlaps)
...
### Task 1: Content-Type Thresholds Access via ConfigRegistry (0.5 days)
- [ ] Thresholds are already defined in `config/base.yaml` (Story 8.1)
- [ ] Extend `content_type_config.py` with accessor functions only
- [ ] Implement `get_content_thresholds(content_type, dimension, metric)` that reads from ConfigRegistry
- [ ] Validation is handled by Pydantic schema in Story 8.1 (`ContentTypeThresholdsConfig`)
- [ ] Add fallback to generic thresholds if ConfigRegistry not initialized (backward compatibility)
- [ ] Verify threshold ranges defined for: Readability, Sentiment, Burstiness, Perplexity, Advanced Lexical, Figurative Language, Voice, Structure
...
**0. ConfigRegistry Integration (Story 8.1 Dependency)**

Thresholds are defined in YAML (`config/base.yaml`), not hardcoded Python:

```yaml
# config/base.yaml (already created by Story 8.1)
content_types:
  thresholds:
    technical_book:
      readability:
      ...
      sentiment:
        variance:
          excellent: [0.100, 0.250]
          # ...
    academic:
      readability:
        flesch_reading_ease:
          excellent: [20, 50]
          poor: [60, 100]  # Too accessible = poor for academic
      # ...
```

Access via ConfigRegistry:
```python
from writescore.config.registry import ConfigRegistry
from writescore.config.schemas.content_type_thresholds import ContentTypeThresholdsConfig
...
def get_content_thresholds(
    ...
    """Get content-specific thresholds from config."""
    try:
        config = ConfigRegistry.get(ContentTypeThresholdsConfig)
        content_thresholds = config.thresholds.get(content_type)
        if not content_thresholds:
            return None  # Falls back to generic thresholds

        dimension_thresholds = content_thresholds.get(dimension)
        ...
        # ConfigRegistry not initialized - fall back to generic thresholds
        return None
```

**1. CONTENT_TYPE_THRESHOLDS (Reference - values in YAML)**
```python
# writescore/core/content_type_config.py (EXTEND)
# NOTE: This shows the STRUCTURE, actual values are in config/base.yaml

# Example of what's in YAML:
...
| 2025-12-13 | Sarah (PO) | Added Story 8.1 dependency; thresholds moved to YAML via ConfigRegistry | APPROVED |
