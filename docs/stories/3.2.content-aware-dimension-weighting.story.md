...
**Dependencies:** Story 3.1 (Content Type Detection), Story 8.1 (Configuration Over Code) - weights defined in YAML
...
### AC1: Content-Type Weights from ConfigRegistry (via Story 8.1)
- [ ] Weight profiles defined in `config/base.yaml` under `content_types.weights` (NOT hardcoded Python dict)
- [ ] Access weights via `ConfigRegistry.get(ContentTypeWeightsConfig)`
- [ ] Includes weight profiles for all 9 content types
- [ ] Weights sum to 1.0 for each content type (validated by Pydantic schema)
...
### Task 1: Content Type Weights Access via ConfigRegistry (0.5 days)
- [ ] Weights are already defined in `config/base.yaml` (Story 8.1)
- [ ] Create `writescore/core/content_type_config.py` with accessor functions only
- [ ] Implement `get_content_weights(content_type: str) -> Dict[str, float]` that reads from ConfigRegistry
- [ ] Validation is handled by Pydantic schema in Story 8.1 (`ContentTypeWeightsConfig`)
- [ ] Add fallback to balanced weights if ConfigRegistry not initialized (backward compatibility)
...
Weights are defined in YAML (`config/base.yaml`), not hardcoded Python:
...
from writescore.config.schemas.content_type_weights import ContentTypeWeightsConfig

def get_content_weights(content_type: str) -> Optional[Dict[str, float]]:
    """Get dimension weights for content type from config."""
    try:
        config = ConfigRegistry.get(ContentTypeWeightsConfig)
        weights = config.weights.get(content_type)
        if weights is None:
            logger.warning(f"Unknown content type: {content_type}, using balanced weights")
            return None
        return weights
    except RuntimeError:
        # ConfigRegistry not initialized - fall back to balanced weights
        logger.info("ConfigRegistry not initialized, using balanced weights")
        return None
```

**1. Content Type Weight Profiles (Reference - values in YAML)**
```python
...
| 2025-12-13 | Sarah (PO) | Added Story 8.1 dependency; weights moved to YAML via ConfigRegistry | APPROVED |
