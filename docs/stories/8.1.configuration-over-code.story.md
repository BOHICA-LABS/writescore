...
16. **Content-Type List Config**: Valid content types (`academic`, `blog`, `technical_book`, etc.) defined in config, not hardcoded (foundation for Epic 3.1)
...
- [ ] **Task 1**: Create config directory structure and base schema (AC: 1, 2)
...
- [ ] **Task 2**: Implement Pydantic schemas for all config sections (AC: 2, 5, 6, 7, 8)
  ...
  - [ ] Subtask 2.5: Create `schemas/root.py` with `WriteScoreConfig` root schema aggregating all sections
  - [ ] Subtask 2.6: Add comprehensive field descriptions and validation constraints to all schemas

- [ ] **Task 3**: Implement config loader with layered override support (AC: 3, 9, 10)
...
- [ ] **Task 4**: Implement ConfigRegistry singleton (AC: 4, 11, 14)
...
- [ ] **Task 5**: Extract current hardcoded values to base.yaml (AC: 5, 6, 7, 8)
  - [ ] Subtask 5.1: Extract `DIMENSION_MODULE_MAP` to `dimensions.modules` in YAML
  - [ ] Subtask 5.2: Extract `BUILTIN_DIMENSION_PROFILES` to `dimensions.profiles` in YAML
  - [ ] Subtask 5.3: Extract dimension weights from each dimension class to `dimensions.weights` in YAML
  - [ ] Subtask 5.4: Extract all `ScoringThresholds` values to `scoring.thresholds` in YAML
  - [ ] Subtask 5.5: Extract `AnalysisConfig` defaults to `analysis` section in YAML
  - [ ] Subtask 5.6: Extract model names (spaCy, sentence-transformers) to `models` section in YAML

- [ ] **Task 6**: Update core modules to use ConfigRegistry (AC: 12)
  - [ ] Subtask 6.1: Update `dimension_loader.py` to load dimension config from registry (with fallback to hardcoded defaults)
  - [ ] Subtask 6.2: Update `dual_score.py` to load thresholds from registry (with fallback to hardcoded defaults)
  - [ ] Subtask 6.3: Update `analysis_config.py` to support config-based defaults
  - [ ] Subtask 6.4: Update dimension base class to support config-based weight override
  - [ ] Subtask 6.5: Update dimensions using ML models to load model names from config

- [ ] **Task 7**: Integrate ConfigRegistry into CLI (AC: 13)
  - [ ] Subtask 7.1: Add `--config-dir` option to CLI with default to `config/`
  - [ ] Subtask 7.2: Add `WRITESCORE_CONFIG_DIR` and `WRITESCORE_ENV` environment variable support
  - [ ] Subtask 7.3: Initialize ConfigRegistry at CLI startup before analysis
  - [ ] Subtask 7.4: Add `--validate-config` command to validate config without running analysis

- [ ] **Task 8**: Create test infrastructure (AC: 14)
  - [ ] Subtask 8.1: Create `tests/fixtures/config/` with test-specific config files
  - [ ] Subtask 8.2: Add `reset_config` autouse fixture to `conftest.py`
  - [ ] Subtask 8.3: Add `test_config` fixture for loading test configuration
  - [ ] Subtask 8.4: Update existing tests to work with config registry (if impacted)
  - [ ] Subtask 8.5: Add unit tests for config loader, registry, and schema validation

- [ ] **Task 9**: Add CI validation and documentation (AC: 15)
  - [ ] Subtask 9.1: Add config validation step to CI workflow
  - [ ] Subtask 9.2: Add "Configuration" section to README with structure overview
  - [ ] Subtask 9.3: Document all config options with descriptions and defaults
  - [ ] Subtask 9.4: Add examples for common customization scenarios (academic, business profiles)

- [ ] **Task 10**: Implement content-type configuration schemas (AC: 16, 17, 18, 19)
  - [ ] Subtask 10.1: Create `schemas/content_types.py` with `ContentTypesConfig` (list of valid content types)
  - [ ] Subtask 10.2: Create `schemas/content_type_weights.py` with weight profiles for all 9 content types
  - [ ] Subtask 10.3: Create `schemas/content_type_thresholds.py` with EXCELLENT/GOOD/POOR ranges for all content types
  - [ ] Subtask 10.4: Add validation: weights sum to 1.0 ± 0.01 for each content type
  - [ ] Subtask 10.5: Add validation: threshold ranges have min < max, no overlaps
  - [ ] Subtask 10.6: Extract content-type weights from Story 3.2 spec to `content_types.weights` in YAML
  - [ ] Subtask 10.7: Extract content-type thresholds from Story 3.3 spec to `content_types.thresholds` in YAML
  - [ ] Subtask 10.8: Add accessor functions: `get_content_type_weights()`, `get_content_type_thresholds()`
  - [ ] Subtask 10.9: Add unit tests for content-type config validation
...
content_types:
  ...
  weights:
    ...
    # ... (professional_bio, personal_statement, technical_docs, business, creative, news)
...
- `.github/workflows/*.yml` - Add config validation step
