...
**Dependencies:** Story 8.1 (Configuration Over Code) - provides ConfigRegistry and content-type list in YAML
...
- [ ] Load valid content types from ConfigRegistry (`config.content_types.types`) instead of hardcoded enum
- [ ] Implement `get_valid_content_types() -> List[str]` that reads from config
...
- [ ] Validate against `get_valid_content_types()` from ConfigRegistry (not hardcoded list)
- [ ] Update main.py to call detector when flag absent
- [ ] Display content type in report header
- [ ] Error message lists valid types from config: `Invalid content type. Valid options: {config.content_types.types}`
...
from writescore.config.schemas.content_types import ContentTypesConfig

def get_valid_content_types() -> List[str]:
    """Get valid content types from config (not hardcoded)."""
    config = ConfigRegistry.get(ContentTypesConfig)
    return config.types  # ['academic', 'blog', 'technical_book', ...]
```
...
class ContentTypeDetector:
    def __init__(self):
        self.valid_types = get_valid_content_types()  # From config
...
| 2025-12-13 | Sarah (PO) | Added Story 8.1 dependency; updated to use ConfigRegistry for content types | APPROVED |
